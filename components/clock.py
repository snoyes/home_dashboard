...
def draw(screen, rect):
    ...
    date_text_rect.topleft = (rect.x + padding, rect.y + padding)
    screen.blit(date_text, date_text_rect)

    # Draw Time
    time_text = time_font.render(time(), 1, colors.white)
    time_text_rect = time_text.get_rect()
    time_text_rect.topleft = (rect.x + padding, date_text_rect.y + date_text_rect.height + date_text_rect.height//2)
    ...
    ampm_text_rect.topleft = (rect.x + padding + time_text_rect.width + fonts.font_4xl.get_height()//2, time_text_rect.y + (y_offset_time - y_offset_ampm))
